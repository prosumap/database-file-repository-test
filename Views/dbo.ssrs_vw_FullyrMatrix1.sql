...
CREATE view [dbo].[ssrs_vw_FullyrMatrix1]
as
select convert (varchar(10),a.FiscalWk) as FiscalWk,c.[MonthDesc],c.Qtr,round(a.[dmd $],0) as [Dmd $],round(b.[Pln Dmd $],0) as [Pln Dmd $],Concat(round(((a.[dmd $]/b.[Pln Dmd $])-1)*100,1),'%')as [Pln %],v.[Bud Dmd $],Concat(round(((a.[dmd $]/v.[Bud Dmd $])-1)*100,1),'%') as [Bud %],ly.[LY Dmd $],
...
round(tblprojected.total,0) as [Proj. BO Fill $]
...
join
(
select fiscalwk,t.total from reftbl_calendar left outer join (select [expected fill fiscal wk], sum([Expected Fill $]) as total  from backorder_0810_1 where [expected fill fiscal Year]  =2020  group by [expected fill fiscal wk]) as t  on t.[expected fill fiscal wk]=FiscalWk group by FiscalWk,t.total
) as tblprojected on  tblprojected.FiscalWk=a.fiscalwk 
join
(select [MonthDesc],FiscalWk,Qtr from [RefTbl_Calendar]  where [Calendar Year]=2020 group by [MonthDesc],FiscalWk,Qtr ) as c on c.FiscalWk=a.FiscalWk 
GO
